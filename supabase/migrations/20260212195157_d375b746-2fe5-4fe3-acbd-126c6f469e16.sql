...
-- 1. Limpeza de dados (ordem respeitando dependências)
DELETE FROM cross_docking;
DELETE FROM docas WHERE status != 'livre';
UPDATE docas SET carga_id = NULL, senha_id = NULL, conferente_id = NULL, volume_conferido = NULL, rua = NULL, status = 'livre';
DELETE FROM cargas;
DELETE FROM senhas;
...
WHERE NOT EXISTS (SELECT 1 FROM cargas c WHERE c.senha_id = s.id);
...
BEGIN
  ...
  IF p_novo_status = 'recusado' THEN
    IF v_carga_id IS NOT NULL THEN UPDATE cargas SET status='recusado' WHERE id=v_carga_id; END IF;
    IF v_senha_id IS NOT NULL THEN UPDATE senhas SET status='recusado', local_atual='aguardando_doca', doca_numero=NULL WHERE id=v_senha_id; END IF;
    UPDATE docas SET status='livre', carga_id=NULL, senha_id=NULL, conferente_id=NULL, volume_conferido=NULL, rua=NULL
      WHERE (v_senha_id IS NOT NULL AND senha_id=v_senha_id) OR (v_carga_id IS NOT NULL AND carga_id=v_carga_id);
    RETURN;
  END IF;
  ...
  IF p_novo_status='em_conferencia' THEN
    IF v_carga_id IS NOT NULL THEN
      UPDATE cargas SET status='em_conferencia', conferente_id=COALESCE(p_conferente_id, conferente_id), rua=COALESCE(p_rua, rua) WHERE id=v_carga_id;
    END IF;
    IF v_senha_id IS NOT NULL THEN UPDATE senhas SET status='em_conferencia' WHERE id=v_senha_id; END IF;
    UPDATE docas SET status='em_conferencia', conferente_id=COALESCE(p_conferente_id, conferente_id), rua=COALESCE(p_rua, rua)
      WHERE (v_senha_id IS NOT NULL AND senha_id=v_senha_id) OR (v_carga_id IS NOT NULL AND carga_id=v_carga_id);
    RETURN;
  END IF;
  ...
  IF p_novo_status='conferido' THEN
    IF v_carga_id IS NOT NULL THEN
      UPDATE cargas SET status='conferido', volume_conferido=COALESCE(p_volume_conferido, volume_conferido),
      conferente_id=COALESCE(p_conferente_id, conferente_id), rua=COALESCE(p_rua, rua), divergencia=COALESCE(p_divergencia, divergencia)
      WHERE id=v_carga_id;
    END IF;
    IF v_senha_id IS NOT NULL THEN UPDATE senhas SET status='conferido' WHERE id=v_senha_id; END IF;
    UPDATE docas SET status='livre', carga_id=NULL, senha_id=NULL, conferente_id=NULL, volume_conferido=NULL, rua=NULL
      WHERE (v_senha_id IS NOT NULL AND senha_id=v_senha_id) OR (v_carga_id IS NOT NULL AND carga_id=v_carga_id);
    RETURN;
  END IF;
  ...
  IF p_novo_status='aguardando_conferencia' THEN
    IF v_carga_id IS NOT NULL THEN UPDATE cargas SET status='aguardando_conferencia' WHERE id=v_carga_id; END IF;
    RETURN;
  END IF;
END;
$$;
