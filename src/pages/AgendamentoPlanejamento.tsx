...
import { fornecedores, statusCargaLabels, tipoCaminhaoLabels } from '@/data/mockData';
...
export default function AgendamentoPlanejamento() {
  ...
  const selectedFornecedor = useMemo(() => 
    fornecedores.find(f => f.id === formFornecedorId),
  [formFornecedorId]);

  // Filter for future/planning cargas (exclude conferido/recusado/no_show)
  ...
  const datasComCargas = useMemo(() => {
    return cargas
      .filter(c => c.status === 'aguardando_chegada')
      .map(c => parseISO(c.data));
  }, [cargas]);
  ...
  const handleEdit = (carga: Carga) => {
    ...
    setFormData(parseISO(carga.data));
    setFormFornecedorId(carga.fornecedorId);
    setFormNfs(carga.nfs.join(', '));
    setFormVolume(carga.volumePrevisto.toString());
    setFormHorario(carga.horarioPrevisto || '08:00');
    setModalOpen(true);
  };

  const handleSave = () => {
    if (!formData || !formFornecedorId || !formVolume) {
      toast.error('Preencha todos os campos obrigatórios');
      return;
    }

    if (editingCarga) {
      atualizarCarga(editingCarga.id, {
  ...
  const handleCancelar = (carga: Carga) => {
    atualizarCarga(carga.id, { status: 'recusado' });
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Planejamento de entregas futuras
              </p>
            </div>
          </div>
                ...
                {cargasFiltradas.length === 0 ? (
                  ...
                  cargasFiltradas.map((carga) => (
                    <TableRow key={carga.id}>
                      ...
                      <TableCell>
                        {carga.tipoCaminhao ? tipoCaminhaoLabels[carga.tipoCaminhao] : '-'}
                      </TableCell>
                      <TableCell>
        ...
        <Dialog open={modalOpen} onOpenChange={setModalOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingCarga ? 'Editar Agendamento' : 'Novo Agendamento'}
              </DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
                ...
                <Popover open={openCalendar} onOpenChange={setOpenCalendar}>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      mode="single"
                      selected={formData}
                      onSelect={(date) => { setFormData(date); setOpenCalendar(false); }}
                      locale={ptBR}
                      className="pointer-events-auto"
                    />
                  </PopoverContent>
                          ...
                          {fornecedoresAtivos.map((f) => (
                            ...
                            >
                              <Check
                                className={cn(
                                  "mr-2 h-4 w-4",
                                  formFornecedorId === f.id ? "opacity-100" : "opacity-0"
                                )}
                              />
                              {f.nome}
                            </CommandItem>
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="nfs"
                  value={formNfs}
                  onChange={(e) => setFormNfs(e.target.value)}
            ...
            <DialogFooter>
              ...
              <Button onClick={handleSave} disabled={!formFornecedorId || !formVolume || !formData}>
                Salvar
              </Button>
            </DialogFooter>
          </DialogContent>
